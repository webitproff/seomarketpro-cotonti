...
**Enhances the SEO capabilities of the Market PRO module in Cotonti by adding meta tags, Open Graph, Twitter Cards, JSON-LD Schema.org structured data, keyword extraction, reading time estimation, and related products functionality.** 
...
![seomarketpro-cotonti_2](https://github.com/user-attachments/assets/e1a6dbcf-216d-41d3-ae2e-25c492c25223)


It helps products appear better in search engines like Google or Yandex.

We enhance the SEO capabilities of the MarketPRO module in Cotonti by adding meta tags, Open Graph, Twitter Cards, structured data in JSON-LD Schema.org, keyword extraction, reading time estimation, and related products functionality.
...
The plugin runs on PHP 8.4+ and MySQL 8.0+/MySQL 8.4.
It is compatible with image extensions (files module or attacher plugin) and reviews (marketproreviews).
In the settings, you can specify a logo, placeholder for images, maximum number of related products, and currency (e.g., USD, EUR, BTC).
...
Navigation paths (breadcrumbs). Improvements for users and search.
...
The plugin automatically selects the first product image and uses it in meta tags and recommendations.
If there's no photo — it uses a placeholder.

With Seo Market Pro and MarketPRO v.5+, products get proper markup, which increases the chances of good positions in search.
Installation is simple, setup takes a couple of minutes.
